...
// Моё понимание победы по диагонали:

// Диагональю считается две линии:
//  X|_|_|_|X|
//  _|X|_|X|_|
//  _|_|X|_|_|
//  _|X|_|X|_|
...
// То есть для победы по диагональной линии необходимо поставить 4 символа в ряд (4 - исходя из условия) по этим линиям.
// Остальные условия для победы - поставить 4 символа в ряд по горизонтали или вертикали

// Я использовал два новых для определения победы:
...
public class les_3_task_3 {
    ...
    private static void humanTurn() {
        ...
        int y;
        do {
            System.out.println("Введите координаты хода (от 1 до 5) через пробел (строка _ столбец) >>>");
            x = SCANNER.nextInt() - 1;
            y = SCANNER.nextInt() - 1;
    ...
    public static boolean checkLine(int start_x, int start_y, int dx, int dy, char dot_type) {
        int countErr = 0;//счётчик ошибок (для случаев когда 0|X|X|X|X или X|X|X|X|0 )
        int countRow = 0;//счётчик подряд идущих символов (для победы нужно 4)
